...
function search() {
  ...
  if (filteredRecipes.length === 0 && query !== '') {
    recipeContainer.innerHTML =
      '<p style="text-align: center; padding: 40px 20px; font-size: 1.2rem; color: #666;">No recipes found matching your search. Try a different keyword.</p>';
  } else {
...
function tagTemplate(tags) {
  return tags
    .map(function (tag) {
      return `<span class="tag">${tag.toLowerCase()}</span>`;
    })
    .join('');
}
...
function init() {
  //random recipe when page loads
  recipeContainer.innerHTML = '';
  const randomIndex = Math.floor(Math.random() * recipes.length);
  renderRecipe(recipes[randomIndex]);
  console.log('Loaded random recipe index:', randomIndex);
}
